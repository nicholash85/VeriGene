...
for filesSucceed in Eugene/Uninfected/*.eug; do
  let "NumRunSuccess += 1"
done

for files in Verilog/Uninfected/*.v; do
  fileRun=false
  ...
  echo "$files" >> logUninf.txt
  echo "Running file: $files"

  fileRunFile="filesRunUninf.txt"
  ...
  then
    ...
    then
      ...
      #then
      failedFile="$files"
      echo "$failedFile" >> failedUninf.txt
